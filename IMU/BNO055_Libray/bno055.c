 ...
 *      Author: HUT HOKKEY (Version Update from GitHub)
 *      IMU BNO055 use to find: Quaternion, Gravity, Eulerangle, Linear Acceleration.
...
esp_err_t bno055_set_default_conf(bno055_config_t * p_bno_conf){
    ...
    p_bno_conf->use_ext_oscillator = 0; // Use external oscillator
...
//-----------
esp_err_t _bno055_buf_to_eulerangle(uint8_t *buffer,bno055_euler_t* eulerangle) {

	int16_t row, pitch, yaw;
    // combine MSB and LSB into 16-bit int
	row   = (((uint16_t)buffer[1])  << 8) | ((uint16_t)buffer[0]);
	pitch = (((uint16_t)buffer[3])  << 8) | ((uint16_t)buffer[2]);
    yaw   = (((uint16_t)buffer[5])  << 8) | ((uint16_t)buffer[4]);

//    const double conv_coeff = 16;  // we assume m/s^2 units
    const double conv_coeff = 1.0/16;
    eulerangle->row   = conv_coeff *row;
    eulerangle->pitch = conv_coeff *pitch;
    eulerangle->yaw   = conv_coeff *yaw;

    return ESP_OK;
}
esp_err_t bno055_get_eulerangle(i2c_number_t i2c_num, bno055_euler_t* eulerangle){
    esp_err_t err = bno055_read_data(i2c_num, BNO055_EULER_H_LSB_ADDR, x_buffer, 6);
    if( err != ESP_OK ) return err;

    return _bno055_buf_to_eulerangle(x_buffer, eulerangle);
}
//----------
esp_err_t bno055_get_fusion_data(i2c_number_t i2c_num, bno055_quaternion_t* quat, bno055_vec3_t* lin_accel, bno055_vec3_t* gravity ,bno055_euler_t* eulerangle){
    ...
    _bno055_buf_to_gravity(x_buffer+16, gravity);
    _bno055_buf_to_eulerangle(x_buffer+24, eulerangle);
